...
Description
-----------
The userpoints2 and userpoints2_nc module provides the ability for users to gain
...
----
Using modules from the project http://drupal.org/project/userpoints2_contrib 
...
2. Upload the userpoints2 directory and all its contents to your
   modules directory.
...
1. Go to Admin -> Modules, and enable userpoints2.
   Check the messages to make sure that you did not get any errors
   on database creation.

2. Go to Admin -> Settings -> userpoints2.
...
userpoints2_userpoints2api()
  ...
  'display'     => (boolean) Whether or not to display the "Points awarded"
                             message. If null, fall back to userpoints2_DISPLAY_MESSAGE
  'tid'         => (int) Taxonomy ID to place these points into; MUST BE in
                         the userpoints2 Vocabulary!

  Examples
    //Add 5 points to the currently logged in user
    userpoints2_userpoints2api(5);  
...
//---Hooks
hook_userpoints2($op, $params = array()) 
  ...
  $op: The operation to be acted upon.
    'setting'
      Pass a field set and fields that would be displayed in the userpoints2
   ...
   The $params variable is the original $params array as sent to userpoints2_userpoints2api
 
//---Other useful functions

userpoints2_get_current_points($uid = NULL, $tid = NULL);
...
userpoints2_get_max_points($uid = NULL, $tid = NULL);
  Returns an integer of the sum of the user's max points achieved
  If a tid is passed in that category's sum is returned otherwise
  the sites default category is used

userpoints2_get_vid()
  Returns an integer of the userpoints2 Vocabulary

userpoints2_get_default_tid()
  Returns an integer for the userpoints2 default Taxonomy ID
  Note: this is the default when submitting points so you 
        DO NOT need to pass this into userpoints2_userpoints2api

userpoints2_get_categories()
  ...
  See userpoints2.module admin_settings function for an example.

userpoints2_get_default_expiry_date()
...
Using the userpoints2_services module, and the services modules, you 
...
userpoints2 provides the following XML-RPC calls:
...
Example:
  ...
  $result = xmlrpc($server_url, 'userpoints2.get', $key, $uid, $tid);
  // $result is an array
  // 'points' => 123
...
Example:
  ...
  $result = xmlrpc($server_url, 'userpoints2.points', $key, $uid, $points, $tid, $event, $description);
  // $result is an array
  // 'status'
  //   1 => Success
  //   0 => Fail
...
http://drupal.org/project/userpoints2
